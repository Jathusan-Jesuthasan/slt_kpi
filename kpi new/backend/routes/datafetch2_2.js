    ...
    for (const month of months) {
      for (const platform of platforms) {
        ...
        try {
          ...
          console.error(`Error fetching data for ${month}, ${platform}:`, innerError);
...
router.get('/data', async (req, res) => {
  try {
    ...
    console.error('TEST ERROR','Error fetching data:', error);
    res.status(500).json({ message: 'Error fetching data-NEW', error });
  }
});
