...
const fetchDataAndStore = async (model, url, platform) => {
  try {
    ...
    console.error(`[ERROR] Fetching or storing data for ${platform} failed: ${error}`);
  }
};
...
export const fetchAndStoreAllData = async () => {
  ...
  await Promise.all([
    fetchDataAndStore(FetchMsan, 'https://socapplications.intranet.slt.com.lk/data-fetch2/data?platform=MSAN', 'MSAN'),
    fetchDataAndStore(FetchVpn, 'https:///data-fetch2/data?platform=VPN', 'VPN'),
    fetchDataAndStore(FetchSlbn, 'https://socapplications.intranet.slt.com.lk/data-fetch2/data?platform=SLBN', 'SLBN')
  ]);
