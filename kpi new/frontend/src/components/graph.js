...
<div className="final-tables-container">
  ...
  >
    {[
      ...
      // Ensure row12Data is available and contains only numbers
      if (!row12Data.length || row12Data.some(isNaN)) return null;

      // Get meter values for this category
      const meterValues = category.meters.map(meter => {
        const meterIndex = columns.indexOf(meter);
        return meterIndex !== -1 ? Number(row12Data[meterIndex]) || 0 : 0;
      });

      // Find the maximum value
      const maxMeterValue = Math.max(...meterValues);
        ...
        >
          ...
          >
            {category.meters.map((meter, idx) => {
              const meterIndex = columns.indexOf(meter);
              let meterValue = meterIndex !== -1 ? Number(row12Data[meterIndex]) || 0 : 0;
              const isMax = Math.abs(meterValue - maxMeterValue) < 0.01; // Handling floating point precision
              ...
              return (
                <div key={idx} style={{ width: '120px', textAlign: 'center' }}>
                  <CircularProgressbar
                    value={meterValue}
                    maxValue={102}
                    text={`${meterValue.toFixed(2)}%`}
                    styles={buildStyles({
                      pathColor: isMax ? 'green' : `rgba(62, 152, 199, ${meterValue / 100})`, // Highlight max value in red
                      textColor: isMax ? 'green' : '#000',
                      trailColor: '#d6d6d6',
                      backgroundColor: '#f3f3f3',
                    })}
                  />
                  <p style={{ marginTop: '10px', color: isMax ? 'black' : 'black', fontWeight: isMax ? 'bold' : 'normal' }}>
                    {meter}
                  </p>
                </div>
              );
