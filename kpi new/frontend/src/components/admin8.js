...
const KPIManagementSystem = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/mtnc-routine');
  ...
  const handleAddOrUpdate = async (e) => {
    ...
    try {
      const url = editingId 
        ? `/api/mtnc-routine/update/${editingId}`
        : '/api/mtnc-routine/add';
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`/api/mtnc-routine/delete/${id}`, {
        method: 'DELETE'
      });
  ...
  return (
    ...
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <h1 style={{ textAlign: 'center', color: '#333', marginTop: '150px' }}>Routine MTNC</h1>
      <form
