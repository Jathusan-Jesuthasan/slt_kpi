...
const columns = [
  'NW/WPC-1(CEN/HK/MD)','NW/WPC-2 (CEN/HK/MD)','NW/WPNE','NW/WPSW','NW/WPSE','NW/WPE','NW/WPN','NW/NWPE','NW/NWPW',
...
const exportToExcel = () => {
  ...
  const kpiTableRows = [
    ...
    ["", "", "", "", "", "Weightage", cleanPercentageValue(totalWeight)],
    ["", "", "", "", "", "Total Weightage", "100%"],
  ];

  // Prepare headers for the dynamic table (right side)
  const dynamicTableHeader = [
    ["R-GM", "Metro", ...Array(12).fill("Metro"), "Region 1", ...Array(12).fill("Region 1"), "Region 2", ...Array(10).fill("Region 2"), "Region 3", ...Array(2).fill("Region 3")],
    ["P-DGM", ...Array(6).fill("Metro 1"), ...Array(6).fill("Metro 2"), ...Array(6).fill("WPN & NWP"), ...Array(6).fill("CP & NCP"), ...Array(4).fill("SAB & UVA"), ...Array(6).fill("WPS & SP"), ...Array(2).fill("EP"), ...Array(4).fill("NP")],
    ["NW EE",  ...columns.flatMap((col) => [col, col])],
    ["RTOM AREA", ...columns.flatMap(() => ["Achieved KPI", "Achieved KPI with Weightage"])],
  ];
  

  const dynamicTableRows = Array(kpiTableRows.length).fill(null).map(() => []);

  // Add dynamic rows to match static table row indices
  kpiRes.forEach((kpiItem, index) => {
    const threshold = index === 0 ? threshold1 : threshold2;
    const wpcVal = kpiItem.percentages["NW/WPC"] || "0.00";
    const wg = parseFloat(kpiData.find((item) => item.no === (kpiItem.no || kpiItem.rowNumber))?.weightage) || 0;
    ...
    dynamicTableRows[index] = [
      ...
      ...columns.flatMap((col) => [
        cleanPercentageValue(rAchieved(kpiItem.percentages[col] || "0.00", threshold)),
        cleanPercentageValue(rAchievedW(kpiItem.percentages[col] || "0.00", threshold, wg)),
      ]),
    ];
  });
  ...
  dynamicTableRows[5] = [
    ...
    cleanPercentageValue(rCurrentMonthWithWeightage(averagePlaceholder?.["NW/WPC"] || "0")),
    ...columns.flatMap((col) => [
      cleanPercentageValue(averagePlaceholder?.[col] || "0"),
      cleanPercentageValue(rCurrentMonthWithWeightage(averagePlaceholder?.[col] || "0")),
    ]),
  ];
  ...
  dynamicTableRows[6] = [
    ...
    cleanPercentageValue(rServFulOkWithWeightage(servFulOkRow?.["cenhkmd"] || "0")),
    ...columns.flatMap((col) => {
      const key = columnToKeyMap[col];
  ...
  dynamicTableRows[9] = [
    ...
    cleanPercentageValue(rSumRowWithWeightage(columnSums[columnSums.length - 1] || "0")),
    ...columns.flatMap((_, i) => [
...
dynamicTableRows[10] = [
  ...
  (() => {
    ...
    const sum = targetRows.reduce((total, rowIndex) => {
      ...
      const numericValue = parseFloat(rowValue.toString().replace('%', '')) || 0;
  ...
  ...columns.flatMap((_, colIndex) => {
    ...
    const sum = targetRows.reduce((total, rowIndex) => {
      const rowValue = dynamicTableRows[rowIndex - 1]?.[withWeightageIndex] || "0";
      const numericValue = parseFloat(rowValue.toString().replace('%', '')) || 0;
      return total + numericValue;
    }, 0);
    return ["", cleanPercentageValue(sum)];
...
dynamicTableRows[11] = [
  ...
  "",
  (() => {
    const sumValue = parseFloat((dynamicTableRows[10][2] || "0").toString().replace('%', '')) || 0;
    // Convert totalWeight to string before using replace
    const weightage = parseFloat(totalWeight?.toString().replace('%', '')) || 0;
    const percentage = (sumValue / weightage) * 100;
    return cleanPercentageValue(percentage);
  })(),
  ...columns.flatMap((_, colIndex) => {
    const withWeightageIndex = (colIndex * 2) + 4;
    const sumValue = parseFloat((dynamicTableRows[10][withWeightageIndex] || "0").toString().replace('%', '')) || 0;
    // Convert totalWeight to string before using replace
    const weightage = parseFloat(totalWeight?.toString().replace('%', '')) || 0;
    const percentage = (sumValue / weightage) * 100;
    return ["", cleanPercentageValue(percentage)];
  ...
  const mergedTableData = kpiTableRows.map((staticRow, i) => {
    const dynamicRow = dynamicTableRows[i] || Array(dynamicTableHeader[3].length).fill("");
  ...
  const finalData = [
    ...
    ["#", "Perspectives", "Strategic Objectives (KRA)", "Key Performance Indicators (KPI)", "Unit", "Description of KPI", "Weightage",
     "RTOM AREA", "Achieved KPI", "Achieved KPI with Weightage", ...columns.flatMap(() => ["Achieved KPI", "Achieved KPI with Weightage"])], // Static table header merged with RTOM AREA row
...
headerRows.forEach((rowIndex) => {
  ...
  row.eachCell((cell, colNumber) => {
    if (cell.value) { // Ensures only header cells are styled
  ...
  // Set column widths
  worksheet.columns = Array.from({ length: finalData[0].length }, () => ({ width: 15 }));
                ...
                <>
                  {kpiData.map(o => (
                    <tr key={o._id}>
                      ...
                      <td style={{textAlign:'left'}}><b>{o.keyPerformanceIndicators || '-'}</b></td>
                      <td>{o.unit || '-'}</td>
        ...
        <div className="final-table-container">
          ...
          <table className="final-distribution-table">
            <thead>
              ...
              <tr style={{ height: '1px' }}>
                <th>R-GM</th><th colSpan="12">Metro</th>
                <th colSpan="12">Region 1</th><th colSpan="10">Region 2</th><th colSpan="6">Region 3</th>
              </tr>
              <tr>
                <th>P-DGM</th>
                <th colSpan="6">Metro 1</th><th colSpan="6">Metro 2</th>
                <th colSpan="6">WPN & NWP</th><th colSpan="6">CP & NCP</th>
                <th colSpan="4">SAB & UVA</th><th colSpan="6">WPS & SP</th>
                <th colSpan="2">EP</th><th colSpan="4">NP</th>
              </tr>
              <tr>
                <th>NW EE</th>
